...
class SettingsLocalDataSource {
  ...
  Future<db.ReceiptSetting?> getReceiptSettings() async {
    return await (_database.select(_database.receiptSettings)
      ..where((t) => t.id.equals('default'))
    ).getSingleOrNull();
  }

  Future<void> saveReceiptSettings(db.ReceiptSettingsCompanion settings) async {
    await _database.into(_database.receiptSettings).insertOnConflictUpdate(settings);
  }

  Future<String?> getSetting(String key) async {
    final result = await (_database.select(_database.appSettings)
