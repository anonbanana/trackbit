...
class CategoryLocalDataSource {
  ...
  Future<List<domain.Category>> getAllCategories() async {
    final results = await (_database.select(_database.categories)
  ...
  Future<void> insertCategory(domain.Category category) async {
    await _database.into(_database.categories).insert(db.CategoriesCompanion(
  ...
  Future<List<domain.CategoryAttribute>> getAttributesByCategoryType(String categoryType) async {
    final results = await (_database.select(_database.categoryAttributes)
      ..where((t) => t.categoryType.equals(categoryType))
      ..orderBy([(t) => OrderingTerm(expression: t.sortOrder)])
    ).get();
    return results.map((a) => domain.CategoryAttribute(
      id: a.id,
  ...
  Future<void> insertCategoryAttribute(domain.CategoryAttribute attribute) async {
    await _database.into(_database.categoryAttributes).insert(db.CategoryAttributesCompanion(
  ...
  Future<void> clearAttributesByCategoryType(String categoryType) async {
    await (_database.delete(_database.categoryAttributes)
      ..where((t) => t.categoryType.equals(categoryType))
    ).go();
  }
