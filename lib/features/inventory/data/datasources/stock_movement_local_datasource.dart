...
class StockMovementLocalDataSource {
  ...
  Future<List<domain.StockMovement>> getAllMovements() async {
    final results = await (_database.select(_database.stockMovements)
      ..orderBy([(t) => OrderingTerm(expression: t.createdAt, mode: OrderingMode.desc)])
    ).get();
    return results.map(_mapMovement).toList();
  }

  Future<List<domain.StockMovement>> getMovementsByProduct(String productId) async {
    final results = await (_database.select(_database.stockMovements)
  ...
  Future<void> insertMovement(domain.StockMovement movement) async {
    await _database.into(_database.stockMovements).insert(db.StockMovementsCompanion(
