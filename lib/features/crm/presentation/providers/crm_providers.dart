...
final customerPurchaseHistoryProvider = FutureProvider.family<List<Map<String, dynamic>>, String>((ref, customerId) async {
  final result = await ref.watch(crmRepositoryProvider).getCustomerPurchaseHistory(customerId);
  return result.when(success: (d) => d, error: (f) => throw Exception(f.message));
});
