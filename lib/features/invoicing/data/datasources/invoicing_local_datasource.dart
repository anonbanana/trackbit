...
class InvoicingLocalDataSource {
  ...
  Future<List<db.Invoice>> getAllInvoices() async {
    return await (_database.select(_database.invoices)
  ...
  Future<void> deleteInvoice(String id) async {
    await (_database.delete(_database.invoices)
      ..where((t) => t.id.equals(id))
    ).go();
  ...
  Future<List<db.InvoiceItem>> getInvoiceItems(String invoiceId) async {
    return await (_database.select(_database.invoiceItems)
  ...
  Future<int> getInvoiceCount() async {
    return await _database.select(_database.invoices).get().then((r) => r.length);
  }
