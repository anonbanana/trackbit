...
class PosLocalDataSource {
  ...
  Future<String?> insertCustomer(String name, String? phone) async {
    ...
    await _database.into(_database.customers).insert(db.CustomersCompanion(
      id: Value(id),
      name: Value(name),
      phone: Value(phone),
  ...
  Future<Map<String, dynamic>?> getReceiptSettings() async {
    final result = await (_database.select(_database.receiptSettings)
  ...
  Future<String> getNextOrderNumber() async {
    final count = await _database.select(_database.orders).get().then((r) => r.length);
    return 'ORD-${DateTime.now().year}${DateTime.now().month.toString().padLeft(2, '0')}-${(count + 1).toString().padLeft(4, '0')}';
  }
