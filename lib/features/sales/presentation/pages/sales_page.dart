            ...
            data: (total) => Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(DateFormat('MMM dd, yyyy').format(today),
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: Column(
                          ...
                          children: [
                            Text(NumberFormat.currency(symbol: '\$', decimalDigits: 2).format(order.total),
          ...
          data: (detail) {
            ...
            return Padding(
              ...
              child: ListView(
                ...
                children: [
                  Center(child: Text(detail.orderNumber,
                      style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
                  const SizedBox(height: 4),
                  Center(
                    child: Text(DateFormat('MMM dd, yyyy HH:mm').format(detail.createdAt),
                  ...
                  _InfoRow(label: 'Payment', value: detail.paymentMethod.toUpperCase()),
                  ...
                  ...detail.items.map((item) => Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text('${item.quantity.toStringAsFixed(0)} x ${NumberFormat.currency(symbol: '\$', decimalDigits: 2).format(item.unitPrice)}',
                            style: const TextStyle(fontSize: 12)),
                  ...
                  const Divider(height: 16),
                  _InfoRow(label: 'Subtotal', value: NumberFormat.currency(symbol: '\$').format(detail.subtotal)),
                  if (detail.discount > 0)
                    _InfoRow(label: 'Discount', value: '-${NumberFormat.currency(symbol: '\$').format(detail.discount)}',
                        color: AppColors.error),
                  if (detail.tax > 0)
                    _InfoRow(label: 'Tax', value: NumberFormat.currency(symbol: '\$').format(detail.tax)),
                  const Divider(),
                  _InfoRow(label: 'Total', value: NumberFormat.currency(symbol: '\$').format(detail.total),
...
class _InfoRow extends StatelessWidget {
  ...
  const _InfoRow({
    ...
    this.bold = false, this.fontSize = 14, this.color,
  });
